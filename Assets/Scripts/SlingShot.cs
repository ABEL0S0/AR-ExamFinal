...
public class SlingShot : MonoBehaviour
{
    ...
    public Vector3 GetMouseWorldPos()
    {
        ...
        return GameObject.FindWithTag("AR CAMERA").GetComponent<Camera>().ScreenToWorldPoint(mousePoint);
    }
    ...
    void Update()
    {
        if (currentAmmo == null)
            return;
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began && !isDrag)
        {
            RaycastHit hit;
            Ray ray = GameObject.FindWithTag("AR CAMERA").GetComponent<Camera>().ScreenPointToRay(Input.GetTouch(0).position);
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended && isDrag)
        {
            ReleaseAmmo();
            isDrag = false;
        }
    ...
    void GrabAmmo()
    {
        mouseZCoord = GameObject.FindWithTag("AR CAMERA").GetComponent<Camera>().WorldToScreenPoint(currentAmmo.transform.position).z;
